...
void FtpServer::handleFTP()
{
  ...
  else if (!client.connected() || !client)
  {
	...
	if(transferStatus!=0) Serial.print("transferStatus ");
	if(transferStatus!=0) Serial.println(transferStatus);
			...
			if(!root){
				...
				while(file){
					if(file.isDirectory()){
						...
						fn.remove(0,1);
						String tmp1("drwxr-xr-x 1 owner group ");
						String tmp2(" Aug 16 16:31 ");
						data.println(tmp1 + String(file.size()) + tmp2  + String(" ") + fn);
						...
						String fn = String(file.name());
						fn.remove(0,1);
						String tmp1("-rwxr-xr-x 1 owner group ");
						String tmp2(" Aug 16 16:31 ");
						data.println(tmp1 + String(file.size()) + tmp2  + String(" ") + fn);

						// data.println("-rw-------  1 peter        " + fs + " Dec 14 11:22" + String(file.name()) );
						// fn.remove(0, 1);
						// fs = String(file.size());
						// data.println( "+r,s" + fs);
						// data.println( ",\t" + fn );
...
boolean FtpServer::doStore()
{
  if( data.connected() )
  {
		Serial.println("FtpServer::doStore");
		// Avoid blocking by never reading more bytes than are available
    int navail = data.available();
		Serial.print("FtpServer::doStore navail ");
		Serial.println(navail);
...
{
	Serial.println("FtpServer::closeTransfer");
